...
public class Database extends SQLiteOpenHelper {
    ...
    public void findSVKhoa(String khoa, ArrayList<SinhVien> sinhViens)
    {
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "select * from "+TB_SINHVIENS + " where " + COL_SINHVIEN_MAKHOA + " = " + khoa;
        Cursor cursor = db.rawQuery(query,null);
        if(cursor.moveToFirst()) {
            do {
                SinhVien sv = new SinhVien();
    ...
    public void deleteTheoDoi(String note)
    {
        SQLiteDatabase db = this.getWritableDatabase();
        String query = "DELETE FROM tbTheoDoi WHERE theodoi_note=" + note;
        db.execSQL(query);
    }
}
